...
public:
	...
	// 아이템 획득 알림을 스택 맨 위에 하나 추가한다 — AActionPlayerController::Client_NotifyItemAcquired가
	// 호출한다. 실제 알림 컨테이너/스택 로직은 SurvivalStatBarsWidget이 소유한다(Debuffs 세로박스와
	// 같은 화면 영역에서 "디버프가 아래, 알림이 그 위로 쌓이는" 배치를 만들기 위함) — 여기서는 그쪽으로
	// 위임만 한다.
...
protected:
	...
	// 획득 알림 컨테이너를 실제로 소유한 위젯 — WBP_MainHUD 안에 배치된 WBP_SurvivalStatBars_New
	// 인스턴스를 이 이름("SurvivalStatBarsWidget")으로 두면 자동 바인딩된다.
	UPROPERTY(VisibleDefaultsOnly, BlueprintReadOnly, meta = (BindWidgetOptional))
	TObjectPtr<USurvivalStatBarsWidget> SurvivalStatBarsWidget;
